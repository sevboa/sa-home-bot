...
> **Версия:** 0.60.3 · **Статус:** ядро готово · Python 3.11+ · 1222 теста, ruff чисто
...
pipx install "git+https://github.com/sevboa/sa-home-bot.git@v0.60.3"
...
- Узнайте свой `chat_id` и впишите первую `[[subscriptions]]` руками — это
  ...
  бот не отвечает вовсе (см. «Приватный вход»), поэтому спросить у него
  `/whoami` до этого не выйдет. В личке `chat_id` равен вашему `user_id` (его
  покажет, например, [@userinfobot](https://t.me/userinfobot)); id группы
  видно в логе бота при `[logging] level = "DEBUG"`. Дальше `/whoami` работает
  как обычно, а остальных проще звать через `/invite`.
...
`/invite` выпускает код и ссылку `t.me/<bot>?start=КОД` (годен час, срабатывает
один раз); погасивший её получает разговор с Альфредом и попадает в список
