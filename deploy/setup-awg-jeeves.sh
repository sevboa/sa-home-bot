...
#
# Порт: для серверов В США/за океаном, к которым ходят из РФ, брать 443
# (ListenPort 443/udp — трафик маскируется под QUIC). Диагностика 2026-09-06:
# на wooster (RackNerd US) из РФ хендшейк на 51820 проходил, а данные ТСПУ
# резал в ноль (US-маршрут душится по QoS); перевод на 443/udp — обход.
# Для NL/EU (jeeves) хватает 51820. endpoint_port в [vpn] config.toml ноды
# ОБЯЗАН совпадать с этим портом.
#
