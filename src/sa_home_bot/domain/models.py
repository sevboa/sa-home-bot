...
class DiskSummary:
    ...
    Собирается фоново раз в scan_cron (`SensorScanJob`) и кэшируется в БД —
    `MonitorService.get_state()` его больше не считает вживую (живой smartctl/
    LHM-опрос на каждый запрос делал `/status`/`/swarm` заметно медленными,
    особенно с тормозящим USB-диском или на Windows). `health`/`temperature_c`
    = None, если SMART недоступен (у eMMC — физически; у прочих — нет прав/
    программы, см. requirements).
