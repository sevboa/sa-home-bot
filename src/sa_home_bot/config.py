...
class GpuSensorConfig(_BaselineParams):
    ...
    исключение (на 2026-09 — только mycraft, Tesla V100; RTX 3060 стояла
    временно на период настройки Tesla и снята). Держать
...
class GraphMemoryConfig(BaseModel):
    ...
    ДОПОЛНЕНИЕ к `memory`, не замена: `memory` не меняется по содержанию.
    Пинуется на mycraft (там живут Neo4j и Ollama), как и `memory`/`net`
    с Этапа 42.1 — mycraft штатно уходит в suspend при простое GPU, и
    недоступность graph_memory (как и memory/net) в это время —
    предусмотренная деградация, не авария (см. protocol.py).
...
class NetConfig(BaseModel):
    ...
    не заводим: limiter SearXNG (единственное, ради чего Redis нужен
    по-настоящему) не требуется — слушаем только localhost, запросов снаружи
    нет.
...
class RealityTransportConfig(BaseModel):
    """Параметры серверного inbound VLESS+Reality (xray-core) — второй
    транспорт службы `vpn` (подэтап 39.0.x, см.
    ``~/.claude/plans/functional-imagining-otter.md``).

    Reality делает настоящий TLS-хендшейк к чужому ``sni``/``dest``, для DPI
    неотличимо от обычного HTTPS — единственный транспорт, реально проходящий
    ТСПУ из РФ на зарубежный сервер.

    Значения ``endpoint_host``/``port``/``server_public_key``/``short_id``/
    ``sni``/``flow`` печатает ops-скрипт ``deploy/setup-reality-server.sh``.
    Имена полей совпадают с ``reality/client_config.py::RealityParams`` —
    ...
    Квоты и учёт — ОБЩИЕ с AmneziaWG (поля ``base_quota_gb`` и т.д. на
    ``VpnConfig``): одна квота на гостя на сервер, независимо от транспорта.
    """
...
class VpnConfig(BaseModel):
    ...
    # --- Транспорты (подэтап 39.0.x). Какие транспорты держит служба vpn на
    # ЭТОЙ ноде: "awg" (AmneziaWG, нужен интерфейс + sudoers + прокси-фиксапы),
    # "reality" (VLESS+Reality через xray, секция [vpn.reality] обязательна).
    # jeeves — ["awg"]; wooster — ["reality"]; нода может нести оба. Квота и
    # учёт трафика общие для всех транспортов (одна квота на гостя на сервер).
...
class VpnCheckConfig(BaseModel):
    ...
    Какие (сервер, транспорт) реально проверять и через какой netns —
    больше не здесь (было ``netns``/``iface``/``probe_server``/
    ``probe_transport`` — ровно ОДНА пара, настраивалась вручную). С
    39.0.7(d) список автообнаруживается (``bot/vpn_nodes.py::probe_targets``
    — все живые VPN-серверы кроме себя) и записывается ``node/fixups.py``
    (``nodectl fix``) в ``node/vpn_probe_state.py::STATE_PATH`` — служба
    только читает этот файл (``VpnCheckService._slots``), в TOML настраивать
    нечего. Каждый слот несёт свой netns + veth-пара + NAT на хосте —
    изоляция от основной маршрутизации ноды не нужны статические маршруты
    под меняющиеся IP целей (api.telegram.org), и туннель не трогает
    обычный трафик ноды. Путь наружу у netns — своя veth-пара + NAT на
    хосте (см. большой комментарий в fixups.py: без него в netns нет ни
    одного физического интерфейса, WireGuard-хендшейку решительно некуда
    уйти — живая находка 2026-08-17). Одна и та же схема работает
    единообразно на любой ноде, включая ноду, где крутится сам VPN-сервер
    (jeeves) — там же без veth-изоляции адрес клиента-пробника совпал бы с
    локальным адресом сервера на этой машине, и ядро отвергало бы
    пересылку как martian source (fib_validate_source), причём независимо
    от rp_filter.
    """
