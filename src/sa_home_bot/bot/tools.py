        ...
        "parameters": {
            ...
            "properties": {
                ...
                "query": {
                    "type": "string",
                    "description": (
                        "Только для search: ТОЛЬКО название фильма/сериала/"
                        "игры, как оно пишется на афише. НЕ добавляй год, "
                        "качество (1080p, WEBRip), сезон, слово «сериал» и "
                        "имя релизера, даже если человек их назвал: трекер "
                        "ищет эту строку целиком в заголовке раздачи, и "
                        "любое лишнее слово обнуляет выдачу. Качество, год и "
                        "размер выбирай потом сам — по именам находок."
                    ),
                },
                "magnet": {
                    "type": "string",
