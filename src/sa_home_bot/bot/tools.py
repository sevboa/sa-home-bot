...
_DECL_NOTIFY_PERSONA: dict[str, Any] = {
    ...
    "function": {
        "name": "notify_persona",
        "description": (
            "Первый шаг перед notify_guest — НО только если владелец САМ НЕ "
            "назвал персонажа явно. Если он попросил конкретно ('от админа', "
            "'как граф', 'от хозяина') — не зови notify_persona вообще, "
            "сразу передай названного персонажа в notify_guest (сверься со "
            "списком допустимых значений в его параметре persona), уважай "
            "явный выбор человека. notify_persona — для случая, когда "
            "персонаж не задан и должен получиться случайным: тул сам его "
            "выбирает и возвращает промт — КАК писать текст в его манере. "
            "Вызови его РОВНО ОДИН РАЗ НА КАЖДОГО получателя, которому "
            "собираешься отправить уведомление notify_guest — если "
...
async def tool_notify_guest(ctx: ToolContext, args: dict[str, Any]) -> str:
    ...
    if persona is None:
        return (
            "ошибка: неизвестный persona — назови конкретного персонажа "
            f"({', '.join(_NOTIFY_PERSONAS_BY_TITLE)}) или сначала вызови "
            "notify_persona и передай сюда его persona как есть"
        )
...
_DECL_NOTIFY_GUEST: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "persona: если владелец САМ назвал персонажа («от админа», «как "
            "граф») — передай его выбор напрямую, НЕ подменяй случайным и не "
            "зови notify_persona. Если персонаж не задан — сначала вызови "
            "notify_persona (на каждого получателя заново) и напиши text в "
            "его стиле, persona здесь передай ровно тем же значением, что он "
            "вернул."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "persona": {
                    ...
                    "description": (
                        "Персонаж — явный выбор человека, если он его назвал, "
                        "иначе ровно то, что вернул notify_persona"
                    ),
                },
                "text": {
                    "type": "string",
                    "description": (
                        "Текст уведомления, написанный в стиле выбранного персонажа"
                    ),
                },
            },
