...
_DECL_NOTIFY_PERSONA: dict[str, Any] = {
    ...
    "function": {
        "name": "notify_persona",
        "description": (
            "Первый шаг перед notify_guest. Тул сам случайно выбирает "
            "персонажа-«отправителя» официального уведомления и возвращает "
            "промт — КАК писать текст в его манере. Вызови его РОВНО ОДИН РАЗ "
            "НА КАЖДОГО получателя, которому собираешься отправить "
            "уведомление notify_guest — если уведомляешь нескольких гостей "
            "подряд, зови notify_persona заново для каждого, не переиспользуй "
            "один и тот же результат: иначе у всех окажется один и тот же "
            "персонаж вместо случайного у каждого. После этого сам сочини "
            "текст уведомления, следуя полученному промту, и передай его в "
            "notify_guest вместе с именем персонажа как есть."
        ),
...
_DECL_NOTIFY_GUEST: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "уведомление?», не выбирай сам. "
            "ОБЯЗАТЕЛЬНО сначала вызови notify_persona (на каждого получателя "
            "заново) и напиши text в его стиле — persona здесь передай ровно "
            "тем же значением, что вернул notify_persona."
        ),
        "parameters": {
            "type": "object",
            "properties": {
