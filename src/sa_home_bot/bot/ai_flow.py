...
# угадать ответ, и мы переспросим уже в режиме рассуждения.
#
# Живая находка 2026-07-24 (живой баг на проде, критично): маркер С
# КИРИЛЛИЧЕСКОЙ "Р" утекал в чат пользователю как есть — "[[ТГЕБУЕТСЯ_
# ГАЗМЫШЛЕНИЕ]]". Причина: llm/service.py детерминированно прогоняет
# ЛЮБОЙ ответ модели (в т.ч. этот служебный маркер) через
# apply_speech_defect (р→г) ДО того, как результат уходит боту — маркер
# приходил уже испорченным, `THINK_MARKER not in fast_answer` не находил
# совпадения, и мангленный маркер отправлялся пользователю как обычный
# ответ. Латиница здесь принципиальна, не косметика: apply_speech_defect
# трогает только кириллические "р"/"Р", strip_math_notation — только
# "$", "{", "}", "\\", "^" и известные LaTeX-команды — ни один из этих
# символов в маркере не встречается, транзит через обе трансформации
# гарантированно не меняет строку.
...
# Живая находка 2026-07-24 (расследовано по ai_tool_calls, см. schema.sql):
# за диалог из 16+ вопросов про время в разных городах get_time реально
# вызвался ОДИН раз — на все "а в москве?"/"а в алмате?"/"какой пояс в
# минске?" модель отвечала из своей (неверной) памяти, хотя тул и таблица
# рабочие. Описание самого тула к этому моменту уже дважды усиливалось
# без эффекта — решено добить тем же приёмом, что сработал для
# THINK_MARKER (пункт 2 ниже): явный чек-пункт прямо перед текущим ходом.
#
# Живая находка 2026-07-25: диалог про кружку с запаянным верхом и
# отпиленным дном — вопрос без единой формулы или числа, поэтому пункт 1
# в прежней формулировке ("формула, геометрия, физика, многошаговый
# расчёт или логическая задача") его не ловил надёжно; модель трижды
# подряд отвечала с ходу и ошибалась. Добавлено явное "пространственная
# головоломка" отдельным словом, а не расчёт на то, что она подпадёт под
# "логическую задачу" — см. также новую секцию _SPATIAL_REASONING в
# llm/prompt.py, которая описывает САМ метод рассуждения для think-режима.
_TRIAGE_INSTRUCTION = (
    "Прежде чем ответить на СЛЕДУЮЩЕЕ сообщение, проверь два пункта.\n"
    "1) Есть ли в нём формула, геометрия, физика, многошаговый расчёт, "
    "логическая задача или пространственная головоломка (например, что "
    "будет с содержимым сосуда/ёмкости при переворачивании, наклоне или "
    "изменении его формы), где легко ошибиться? Если да — ДАЖЕ если тебе "
    "кажется, что ты и так знаешь ответ, НЕ отвечай и НЕ считай сам, а "
    f"выведи ТОЛЬКО одну строку без всего остального: {THINK_MARKER}.\n"
    "2) Спрашивают время, дату, день недели или часовой пояс в конкретном "
    "городе/стране — включая короткие вопросы-продолжения вроде «а в Х?» "
    "сразу после уже обсуждавшегося другого места? Если да — ДАЖЕ если "
    "тебе кажется, что ты и так знаешь смещение этого пояса, ты ОБЯЗАН "
    "вызвать тул get_time в ЭТОМ ЖЕ ходу и ответить только тем, что он "
    "вернул — не отвечай числом времени/пояса из своей памяти. Это "
    "касается КАЖДОГО названного места отдельно, даже если про другое "
    "место тул уже вызывался раньше в этом разговоре.\n"
    "Если ни один из двух пунктов не подходит (простой бытовой/светский "
    "вопрос, тривиальная арифметика вроде «сколько будет 2+2») — отвечай "
    "сразу и как обычно, в характере."
)
