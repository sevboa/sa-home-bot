...
async def request_alfred(
    ...
    # Живая находка 2026-08-10 (второй заход): Агнольд — отдельный персонаж
    # со своей репликой, не эфемерный статус. Его сообщение должно ОСТАТЬСЯ
    # в чате, не перекрываясь ни «шагами», ни ответом самого Альфреда —
    # поэтому всегда персистентный message.answer, независимо от
    # rich_session (первая версия этого фикса пыталась пускать Агнольда
    # через push_status/черновик с паузой на прочтение — но черновик в
    # принципе эфемерен, реплика всё равно потом стиралась следующим
    # статусом/финалом; пауза лечила только видимость, не суть).
