...
# Тот же текст без HTML — для rich-черновика (push_status → thinking-блок,
# см. STEPS_TEXT_PLAIN про ту же пару и почему их два).
ARNOLD_WAKING_PLAIN = "Агнольд: Сейчас Альфред подойдёт"
ALBERT_UNAVAILABLE = (
...
# Минимальное время на экране для статуса Агнольда в rich-черновике — живой
# баг 2026-08-10: при быстром wake (magic packet сработал, wait_for_service
# прошёл с первой попытки) следующий push_status (повторные «шаги» ниже)
# улетал практически сразу следом, без паузы — клиент Telegram не успевал
# отрисовать текст Агнольда, черновик перезаписывался раньше, чем человек
# успевал прочитать («моргнуло и опять шаги»). Фолбэк-путь
# (message.answer) паузы не требует — там Агнольд отдельное персистентное
# сообщение, тайминг ни на что не влияет.
ARNOLD_WAKING_DISPLAY_S = 2.0
...
async def request_alfred(
    ...
    if rich_session is not None:
        # Rich-путь: тот же draft_id, что и «шаги» до этого — Агнольд просто
        # сменяет предыдущий статус в эфемерном черновике (Telegram сам
        # анимирует смену), отдельным персистентным сообщением не становится.
        # Пауза ниже — см. ARNOLD_WAKING_DISPLAY_S: без неё следующий статус
        # («шаги» опять) перезаписывает черновик раньше, чем видно.
        await rich_session.push_status(ARNOLD_WAKING_PLAIN)
        await asyncio.sleep(ARNOLD_WAKING_DISPLAY_S)
    else:
        # Фолбэк (группы/typing_plain): черновиков нет платформенно —
        # «шаги» уже ушли отдельным message.answer, и Агнольд по той же
        # причине не может его заменить, только добавиться следующим.
    ...
    # провалится — это уже шаги Альбегта (см. ALBERT_ASLEEP ниже). В
    # фолбэк-ветке не переиспользуем первое сообщение, чтобы сюжетно оба
    # провала были у разных персонажей, а успех — молча «оказывается, шёл
    # Агнольд»; в rich-ветке тот же эффект даёт замена статуса в черновике.
    await _announce_steps()
    try:
        async with _typing_while_asking(message):
            return await _ask()
