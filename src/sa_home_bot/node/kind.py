...
- jeeves — виртуалка: включена всегда, термозон и SMART у неё нет — монитор там
  мерит host-метрики из /proc (steal/iowait/память/диск), а не температуры
  (этап 38, `sensors/host.py`).
...
class NodeTraits:
    ...
    (С этапа 38 потребителей у флага нет: monitor работает и на vps — host-метрики
    из /proc. Оставлен на случай будущих служб, действительно требующих железо.)
    ...
    def power_controllable(self) -> bool:
        """Можно ли выключать/перезагружать/усыплять эту машину по команде.

        Для always_on ноды (server, vps) недоступность — авария, поэтому
        добровольный увод её в офлайн (даже своей же кнопкой в боте) не
        предлагается вообще: действие не объявляется в describe(). Только
        workstation, которая и так штатно выключается и будится по WoL.
        """
