...
-- Допуск гостя на ЭТОТ сервер и его постоянная личная квота (решение
-- пользователя 2026-09-18). Таблица физически пер-серверная — у каждой ноды
-- vpn своя БД, — поэтому строка тут и означает «гостю открыта эта локация на
-- N ГБ»; отдельной колонки под сервер не нужно.
--
-- allowed = 0 ИЛИ отсутствие строки — не допущен: новые гости закрыты, пока
-- владелец явно не откроет локацию (/vpn → «👥 Все гости»). Тем, у кого пиры
-- уже были на момент появления таблицы, allowed = 1 проставляется разово при
-- старте (vpn/service.py::backfill_access) — обновление не должно никого
-- выставить за дверь.
--
-- base_bytes = NULL — общая база из [vpn].base_quota_gb, иначе личная. База
-- ПОСТОЯННАЯ, в отличие от помесячных vpn_quota_grants: лимит месяца =
-- base + гранты месяца (vpn/service.py::_limit_bytes). Ноль запрещён — «ноль
-- гигабайт» выражается через allowed = 0, иначе used >= limit сработало бы
-- как исчерпанная квота и гость получил бы ⛔️-уведомления ни за что.
CREATE TABLE IF NOT EXISTS vpn_chat_access (
    chat_id     INTEGER PRIMARY KEY,
    allowed     INTEGER NOT NULL DEFAULT 0 CHECK (allowed IN (0, 1)),
    base_bytes  INTEGER CHECK (base_bytes IS NULL OR base_bytes > 0),
