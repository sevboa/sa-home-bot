...
- Портировать сервис ноды + monitor на Windows (датчики: WMI /
  LibreHardwareMonitor; автозапуск — служба Windows). Если Python-монорепа
  на Windows не взлетит — это первый кандидат в отдельный модуль
  `sa-mod-monitor-win` (см. ARCHITECTURE §12).
- Первая межнодовая связь — **по правилу «спроси любого» (ARCHITECTURE §11
  п. 2): нода alfred ходит к ноде ПК**, а не бот к монитору ПК напрямую.
  Бот по-прежнему знает только сокет своей ноды; маршрутизацию к удалённой
  ноде выполняет нода alfred (`node_id` в конверте уже есть). Так мы сразу
  обкатываем межнодовую маршрутизацию, а не вкручиваем её потом.
