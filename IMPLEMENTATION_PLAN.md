...
> отдельный процесс, бот — её клиент; две user-службы systemd в `deploy/`).
> Дальнейшее развитие — см. раздел «Видение и дорожная карта» в конце документа.
...
- **Остаток (перенесён вперёд):** кнопки под `/status` пока захардкожены;
  динамическое построение действий из `describe` (id → кнопка, права
  `действие@нода`) — вместе с этапами 14–15, когда действий станет больше одного.

### Этап 14. Сервис ноды (супервизор) + консоль
