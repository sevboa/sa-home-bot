...
2. Промежуточный тест роя на второй Linux-ноде (рабочий ноутбук Arch) ⏳:
   умение `power` (poweroff/reboot/suspend) и аптайм в get_state ноды —
   ✅ сделаны кроссплатформенно (2026-07-11: команды Linux/Windows в
   `node/service.py`, `[node].id` для имени ноды, `nodectl -n <нода>` +
   power-подкоманды). WoL для ноутбука по Wi-Fi скорее всего не работает —
   «включить» может остаться только у Windows-ПК (Ethernet).
