...
5. Умения управления: `apps-win` (CreateProcessAsUser); дальше — службы
   Windows и автозапуск. Power-кнопки на карточке ноды в боте (сейчас
   действия без параметров на карточку не попадают — только nodectl).

Бот подтянут под рой ✅ (2026-07-11): `/nodes` показывает пиров из
`[[swarm.nodes]]` (лампочка 🟢/🔴), у каждого — карточка «спроси любого»
(`ServiceLink.get_state/command` научился `dst`, `NodeRouter` пересылает).
